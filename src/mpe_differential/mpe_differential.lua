...
local INERTIA_SMOOTHING_FACTOR = 0.5 -- [0..1] low=smooth, high=responsive
...
function onTick()
	...
	if composite_ok then
		...
		INERTIA_SMOOTHING_FACTOR = clamp(floats[3], 0, 1)
		RPS_SLIP_ENGAGEMENT_FACTOR = math.max(floats[4], 0)
		LR_BIAS = clamp(floats[5], -1, 1) -- TODO: Implement bias for torque vectoring
		BASE_INERTIA = math.max(floats[6], 0)
	end
	...
	if driveshaft_a_connected then
		driveshaft_rps = component.slotTorqueApplyMomentum(
			DRIVESHAFT_A,
			(BASE_INERTIA + AXLE_A_INERTIA + AXLE_B_INERTIA) / DRIVE_RATIO,
	...
	-- Apply axle momentum
	local rps_a =
		component.slotTorqueApplyMomentum(AXLE_A, (BASE_INERTIA + DRIVESHAFT_INERTIA) * DRIVE_RATIO, axle_a_target_rps)
	local rps_b =
		component.slotTorqueApplyMomentum(AXLE_B, (BASE_INERTIA + DRIVESHAFT_INERTIA) * DRIVE_RATIO, axle_b_target_rps)
	...
	DRIVESHAFT_INERTIA = DRIVESHAFT_INERTIA * (1 - INERTIA_SMOOTHING_FACTOR)
		+ calculated_driveshaft_inertia * INERTIA_SMOOTHING_FACTOR
	...
	AXLE_A_INERTIA = AXLE_A_INERTIA * (1 - INERTIA_SMOOTHING_FACTOR)
		+ calculated_axle_a_inertia * INERTIA_SMOOTHING_FACTOR
	...
	AXLE_B_INERTIA = AXLE_B_INERTIA * (1 - INERTIA_SMOOTHING_FACTOR)
		+ calculated_axle_b_inertia * INERTIA_SMOOTHING_FACTOR
